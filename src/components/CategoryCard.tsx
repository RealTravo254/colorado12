...
export const CategoryCard = ({
  ...
  return (
    ...
    >
      {bgImage ? (
        <div className="relative w-full aspect-[3/2] md:aspect-[4/3] overflow-hidden rounded-lg md:rounded-xl">
          <img
            src={bgImage}
            alt={title}
            ...
            loading="lazy"
          />
          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent rounded-lg md:rounded-xl" />
          <div className="absolute bottom-0 left-0 right-0 p-1 md:p-2 text-center">
            <h3 className="font-medium text-[8px] sm:text-[10px] md:text-xs text-white uppercase tracking-wider drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">{title}</h3>
          </div>
        </div>
      ) : (
